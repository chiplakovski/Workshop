(function(global){
  ...
  // The demonstration records. Kept so the system can be shown populated on request, and so
  // the test suite has a fixture to exercise every rule against - never loaded on its own.
  const demoState=()=>({
    version:VERSION,
  ...
  const api={
    ...
    ensureDemoEquipment:()=>{
      if(!Array.isArray(state.equipment)||state.equipment.length===0){
        state.equipment=clone(demoState().equipment);
