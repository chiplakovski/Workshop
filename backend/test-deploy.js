...
async function main() {
  ...
  assert.equal(value(`SELECT count(*) FROM information_schema.tables
    WHERE table_schema = 'public' AND table_type = 'BASE TABLE';`), '34');
  assert.equal(value(`SELECT count(*) FROM pg_policies WHERE schemaname = 'public';`), '75');
  ...
  step('Deploy: all four files install as a non-superuser owner — 34 tables, 75 policies, five roles');
  ...
  assert.equal(value(`SELECT count(*) FROM pg_proc p JOIN pg_roles r ON r.oid = p.proowner
    WHERE r.rolname = 'varmak_engine';`), '17',
