...
-- ═══════════════════════════════════════════════════════════════════════════
-- Phase 0A-R1 review-correction manual additions (not generated by Prisma).
...
-- ── SafetyEvent ───────────────────────────────────────────────────────────
-- Exactly one typed safety target per event, matching its kind. DOCUMENT_SUPERSEDED now
-- identifies the two actual Document versions directly (previousDocumentId/newDocumentId),
-- not via QualityHold/QualityRelease alone.
...
-- gateVersion must not be blank (decidedAt and gateResultSnapshot are plain NOT NULL columns —
-- see schema.prisma — so no separate CHECK is needed for "a decision cannot exist without its
-- snapshot"; blockingReasons defaults to '[]' at the column level, so it is never NULL either).
...
-- Release-fact immutability: once status is RELEASED, no release field (including the new
-- releasedActorType) can ever change again. The ACTIVE -> RELEASED transition itself remains a
-- legitimate, required UPDATE — only the release fact is locked once it exists.
CREATE OR REPLACE FUNCTION quality_hold_release_immutable()
RETURNS TRIGGER AS $$
BEGIN
  IF OLD."status" = 'RELEASED' AND (
    NEW."status" IS DISTINCT FROM OLD."status" OR
    NEW."releasedActorType" IS DISTINCT FROM OLD."releasedActorType" OR
